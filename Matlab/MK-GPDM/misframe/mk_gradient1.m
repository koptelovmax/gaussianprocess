...
Ky_RBF1 = Ky1 - eye(size(Xmis, 1))*1/hyperpara_Ky(end); % 由于GPLVM中的computeKernel函数计算的核部分包括了噪声项，这里把噪声项去掉，重新恢复成纯RBF核,用于后面求导
...
N1 = size(Xmis,1);
dL_dx = zeros(N-1, Q);  % 似然函数对隐变量模型求偏导，
for d = 1:Q
    Kpart = kernelDiffX(Xmis, hyperpara_Ky, d, Ky_RBF1); % 直接是dK_y/dx的函数，但是需要每一个维度的求
